...
impl CpuWidget {
    pub fn new(left_top: Coordinate, right_bottom: Coordinate, theme: Theme, sys:&mut System) -> Self {
        Self {
            ...
            coordinate: left_top,
            theme,
            cpu_brand: sys.cpus().first().map(|c| c.brand().to_string()).unwrap_or(consts::UNKNOWN.to_string()),
...
impl Widget for CpuWidget {
    ...
    fn render(&self, stdout: &mut Stdout) -> std::io::Result<()> {
        ...
        queue!(stdout, MoveTo(x,y+1), Print(format!("CPU使用率:     {:>5.2}%", self.cpu_usage)))?;
        queue!(stdout, MoveTo(x,y+2), Print(format!("Core核心数:    {:>6}", self.core_count)))?;
        queue!(stdout, MoveTo(x,y+3), Print(format!("Core物理核心数: {:>5}", self.core_count)))?;
        queue!(stdout, MoveTo(x,y+4), Print(format!("Core频率:      {:>5} MHz", self.frequency)))?;
        Ok(())
    }
}